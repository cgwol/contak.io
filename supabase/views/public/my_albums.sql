...
CREATE OR REPLACE VIEW public.my_albums WITH (security_invoker = true) AS 
...
WHERE auth.uid() IN (SELECT album_creators.creator_id FROM contak.album_creators WHERE album_creators.album_id = albums.album_id);
